...
def decode(sequence):
    ...
    s = ''.join([x for j, x in enumerate(a[:-1]) if x !=
                characters[0] and x != a[j + 1]])
    if len(s) == 0:
        return ''
...
def pred(img_content):
    img = np.asarray(Image.open(BytesIO(img_content)),
...
def get_post_data(soup=None):
    ...
    CaptchaImageUrl = soup.find(
        id="c_status_ctl00_contentplaceholder1_defaultcaptcha_CaptchaImage").attrs["src"]
...
def query_ceac_state(loc, case_no, pp_no, surname, data=None):
    ...
    error_text = soup.find(
        id="ctl00_ContentPlaceHolder1_ValidationSummary1").text.strip()
    if error_text:
    ...
    if error_text:
        return error_text, None
    status = soup.find(
        id="ctl00_ContentPlaceHolder1_ucApplicationStatusView_lblStatus").text
    caseno = soup.find(
        id="ctl00_ContentPlaceHolder1_ucApplicationStatusView_lblCaseNo").text
    SubmitDate = soup.find(
        id="ctl00_ContentPlaceHolder1_ucApplicationStatusView_lblSubmitDate").text
    StatusDate = soup.find(
        id="ctl00_ContentPlaceHolder1_ucApplicationStatusView_lblStatusDate").text
    Message = soup.find(
        id="ctl00_ContentPlaceHolder1_ucApplicationStatusView_lblMessage").text
...
def query_ceac_state_safe(loc, case_no, pp_no, surname, soup=None):
    for _ in range(10):
        try:
            ...
            logger.info("Info!, %s-%s-%s-%s: %s", loc,
                        case_no, pp_no, surname, result)
        except Exception as e:
            logger.error("Error!, %s-%s-%s-%s: %s", loc,
                         case_no, pp_no, surname, e)
            return str(e), None
